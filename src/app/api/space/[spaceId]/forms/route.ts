...
import { AuthError, ExceedLimitError } from "@/utils/lib/errorclass";
...
export async function POST(
	...
	try {
		...
		const maxAllowedTestimonials =
			userDetails.subscription[0].subscriptionData.maxTestimonialForm;

		if (
			maxAllowedTestimonials !== -1 &&
			totalTestimonialForms >= maxAllowedTestimonials
		) {
			throw new ExceedLimitError(
				`You have exceed Your Limit , Please Upgrate to ${userDetails.subscription[0].subscriptionName === "TRIAL" ? "PRO" : "ENTERPRICE"}`,
			);
		}
...
export async function PUT(
	req: NextRequest,
	{ params }: { params: Promise<{ formId: number }> },
) {
	const formId = (await params).formId;

	if (!formId) {
		return NextResponse.json(
			{ msg: "No formId invalid Request" },
			{ status: 403 },
		);
	}
	try {
        ...
        const parsedObject = TestimoniaFormConfig.safeParse(await req.json())

        if(!parsedObject.success){
            throw parsedObject.error
        }

		const { Name, Description } = parsedObject.data;

        const res = await prisma.testimonialForm.update({
            where : {
                id  : formId,
                adminId : Number(id)
            }, data : {
        ...
        return NextResponse.json(
			{ msg: "Testimonia Form Updated Successfully !!" },
			{ status: 200 },
		);
		...
		if (error instanceof ZodError) {
			return NextResponse.json(
				{ msg: `Invalid inputs ${JSON.stringify(error.cause)}` },
				{ status: 400 },
			);
		}
	}
	...
	return NextResponse.json(
		{
			msg: "Something bad happens at backend while Updating Forms",
		},
		{
			status: 500,
		},
