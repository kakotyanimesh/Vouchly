...
import {
	InputBoxesTypesStore,
	useTestimonialSubmissionStore,
} from "@/utils/zustand/submittestimonialstate";
...
export const SubmitFormComponent = (
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (storeNumber === 1 && !videofile) {
			toast.error(
				"Oops, your video`s missing 🎬 Wanna try uploading again?",
			);
			return;
		}
		startTransition(async () => {
			...
			const canupload = await checkReviewUsages({
				adminId: data.adminId,
				reviewType: storeNumber === 0 ? "text" : "video",
			});
			...
			try {
				const testimonialObject: ReviewTypes = {
					...
					customerName,
					customerImageUrl:
						"emapty for now will be uploaded after  s3",
				...
				if (storeNumber === 1) {
					...
					if (!videofile) {
						toast.error(
							"A video is required to submit this type of review 🎥",
							{
								id: toastId,
							},
						);
					...
					const videoLink = await uploadToS3(
						videofile,
						"testimonial-videos",
					);
					...
					if (!videoLink || !videoLink.uniqueKey) {
						toast.error(
							videoLink?.message ||
								"Ugh, video upload didn`t work 😩 Try again?",
							{
								id: toastId,
							},
						);
		...
		<div className="mb-10">
             ...
             */}
					<Image
						...
						width={100}
						height={100}
						className="rounded-2xl max-h-20 object-cover"
						alt="form_logo"
					/>
					<div>
					...
					>
						Built With{" "}
						<span className="bg-gradient-to-r from-[hsl(var(--card-bg-one))] to-[hsl(var(--card-bg-two))] text-transparent bg-clip-text">
							Embriefy
						</span>
					</Button>
				</Card>
