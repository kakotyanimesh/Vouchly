	...
	return (
		...
		>
			<Link
				href={"/"}
				className={cn(
					"font-bold hidden text-white text-2xl gap-2 pl-4 md:flex items-baseline border-b border-[hsl(var(--pure-white))]/10 py-4",
				)}
			>
				<span className="bg_card_gradient rounded-md px-2 pt-1">
					V
				</span>
				{showFullSidebar ? (
			...
			<div className="space-y-2 md:px-3 px-10 md:py-0 py-2 relative flex md:flex-col flex-row justify-between">
				...
				{SideBarRoutes.map((s, k) => (
					<Link
						...
						href={s.src}
						className={cn(
							"cursor-pointer h-10 transition-colors duration-200 ease-linear rounded-xl p-2 flex items-center gap-2 z-10",
							p === s.src
								? "text-white md:bg-transparent bg_card_gradient"
								: "hover:bg-[hsl(var(--pure-white))]/20",
						)}
					>
						{s.icons}
...
const SideBarRoutes = [
	...
	},
	{
		name: "Space",
		src: "/space",
		icons: <Rocket strokeWidth={1.5} size={18} />,
	},
	{
		name: "Forms",
		src: "/forms",
