...
export const SpaceCard : React.FC<SpceCardProps> = ({className, spaceName, createdAt, src, Id, totalForms}) => {
    ...
    return (
        <Card className={cn("px-5 py-7 bg-[hsl(var(--primary))]/10 space-y-5 group hover:bg-[hsl(var(--primary))]/5 hover:border-[hsl(var(--primary))] active:bg-[hsl(var(--primary))]/5 active:border-[hsl(var(--primary))] transition-colors ease-linear duration-200", className)}>
            ...
            variant={"secondary"} className="w-full py-2 group-hover:border-[hsl(var(--primary))] group-active:border-[hsl(var(--primary))]" sizes={"sm"}>Open Space </Button>
        </Card>
    )
}