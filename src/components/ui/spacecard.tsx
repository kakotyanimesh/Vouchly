...
export const SpaceCard : React.FC<SpceCardProps> = ({className, spaceName, createdAt, src, Id, totalForms}) => {
    ...
    return (
        <Card className={cn("px-5 py-7 bg-[hsl(var(--secondary))] space-y-5 group hover:bg-[hsl(var(--primary))]/5 hover:border-[hsl(var(--primary))] active:bg-[hsl(var(--primary))]/5 active:border-[hsl(var(--primary))] transition-colors ease-linear duration-200", className)}>
            <div className="flex flex-row justify-between items-center">
                <div className="flex items-center gap-2">
                    <IconDiv reactNode={<Rocket size={18}/>} className="rounded-md p-2 group-hover:border-[hsl(var(--primary))]"/>
                    <div className="-space-y-1">
                        <h1 className="group-hover:text-[hsl(var(--primary))] transition-colors ease-linear duration-150 font-semibold">{spaceName}</h1>
            ...
            variant={"primary"} className="w-full py-2 group-hover:border-[hsl(var(--primary))] group-active:border-[hsl(var(--primary))]" sizes={"sm"}>Open Space </Button>
        </Card>
    )
}