...
export const DashboardCard : React.FC<DashboardProps> = ({className, title, data, desc, ...props}) => {
    return (
        <Card className={cn("bg-black/40 px-7 py-6 hover:border-[hsl(var(--primary))]/40 active:border-[hsl(var(--primary))]/40 transition-colors ease-linear duration-200", className)} {...props}>
            <h1 className="text-[hsl(var(--card-foreground))]/70 font-semibold">Total {title}</h1>
