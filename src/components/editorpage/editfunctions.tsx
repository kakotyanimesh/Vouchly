...
import {
	...
	ClassicGrid,
	MassonaryGrid,
} from "./exampleedits/exampleedits";
...
export const WidgetCustomizer = ({ formId }: { formId: number }) => {
	const { gridType, setgridType, openFinalWidget, setOpenFinalWidget } =
		useGridStore();
			...
			<div className="grid grid-cols-3 lg:grid-cols-1 gap-3 space-x-2">
				{styleGuideArray.map((s, k) => (
					<Card
						...
						key={k}
						className={cn(
							"md:py-2 text-sm  space-y-1 h-full sm:px-5 p-1 cursor-pointer relative rounded-xl transition-all ease-linear duration-300 hover:bg-[hsl(var(--tertiary))]/5",
							gridType === s.type
								? "shadow-[0px_0px_4px_2px_#cf10f1]"
								: undefined,
						)}
					>
						<h1
							className={cn(
								gridType === s.type
									? "text-[hsl(var(--tertiary))]"
									: "bg-gradient-to-br from-[hsl(var(--primary))] to-[hsl(var(--tertiary))] bg-clip-text text-transparent",
							)}
						>
							{s.title}
						</h1>
						<p className="text-[hsl(var(--secondary-foreground))]/70 text-xs lg:block hidden">
							{s.desc}
						</p>
						{s.comp}
					</Card>
...
const FinalGridCard = ({ formId }: { formId: number }) => {
	...
	const setOpenFinalWidget = useGridStore(
		(state) => state.setOpenFinalWidget,
	);
	...
	return (
		<Card className="xl:left-20 md:inset-y-10 md:right-5 md:left-20 right-1 left-3 top-10 bottom-15 z-20 absolute py-4 lg:px-10 px-5 flex flex-col space-y-2">
			...
			<Button
				variant={"transparent"}
				sizes={"sm"}
				className="absolute md:right-10 right-3 p-0"
				onClick={() => setOpenFinalWidget()}
			>
				<X />
			</Button>
					...
					<InfiniteScrollCard className="flex justify-center h-full">
						{gridType === "Classic" ? (
							<ClassicGridComponent
								orderedReviews={orderedReviewss}
	...
	const saveCustomization = (e: React.FormEvent<HTMLFormElement>) => {
		...
		startTransition(async () => {
			...
			try {
				...
				const saveEmbadedId = await saveScriptKey({
					embadedId: res.id,
					formId: formId,
				});
	...
	return (
		<Card className="flex flex-col p-5 w-96 space-y-5">
			...
			<form onSubmit={saveCustomization} className="space-y-5">
				<h1>
					Don&apos;t forget to change the required Styles & match the
					bg color now its black{" "}
				</h1>
				<DimensionInput
					label="🍩Rounded Vibes"
					icon={<Scan size={14} />}
					initialValue={roundedCorner}
					onChangeFuntion={(e: React.ChangeEvent<HTMLInputElement>) =>
				...
				<ColorPicker
					...
					onChangeFunction={(
						e: React.ChangeEvent<HTMLInputElement>,
					) => setrewiewCardBg(e.target.value)}
				/>
				<ColorPicker
					title="✨Hype Stars"
